...
<title>Listaa veneet | Peppi Mäkelä</title>
</head>
<body>
...
<table id="listaus">
	<thead>
		<tr>
			<th colspan="6" class="oikealle"><span id="uusiVene" class="siirry">LISÄÄ UUSI VENE</span></th>
		</tr>
		<tr>
			<th>Hakusana:</th>
...
<div>
<img src="images/veneen_kuva.jpg" alt="Kuva veneestä">
</div>
...
function poista(tunnus, nimi, merkkimalli) {
	if(confirm("Haluatko varmasti poistaa veneen " + merkkimalli + " nimeltä " + nimi + "?")) {
		$.ajax({
			url: "veneet/" + tunnus,
			type: "DELETE",
			dataType: "json",
			success: function(result) {
				if(result.response == 0) {
		        	...
		        	$("#rivi_" + tunnus).css("background-color", "#FE6B41"); // En saanut tätä toimimaan Google Chromella, mutta Eclipsen sisäisellä selaimella toimi hyvin.
					alert("Veneen " + merkkimalli + " poistaminen onnistui.");
					haeVeneet();
				}
			}
		});
